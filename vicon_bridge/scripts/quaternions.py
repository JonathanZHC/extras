...
def omega_from_quat_quat(q1, q2, dt):
    ...
    if tf.vector_norm(q1 - q2) < 1e-8:
        ...
        return angle / dt * r[:3] / tf.vector_norm(r[:3])
...
def global_to_body(q, vec):
    ...
    return np.dot(tf.quaternion_matrix(q)[:3, :3].transpose(), vec)
